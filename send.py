...
RUNTIME_ENTROPY_STR = RUNTIME_ENTROPY.decode('utf-8')
...
parser.add_argument('-d', '--data', required=False, nargs='?', dest='data', help='Data to send. If empty uses stdin. Must be string compatible.')
...
SHARED_KEY = config.get('Security', 'shared_key') if args.key is None else args.key
...
hashfunc.update(str(SHARED_KEY).encode('utf-8'))
hashfunc.update(str(RUNTIME_ENTROPY_STR).encode('utf-8'))
hashfunc.update(str(TRANSMISSION_COUNTER).encode('utf-8'))
hashfunc.update(str(data).encode('utf-8'))
...
sock.sendto(envelope.encode('utf-8'), (MCAST_GRP, MCAST_PORT))
